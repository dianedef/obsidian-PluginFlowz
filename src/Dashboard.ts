import { ItemView, Plugin, Modal, Notice, Setting, Component, HTMLElement, WorkspaceLeaf } from 'obsidian';
import { IPlugin, TPluginStatus, TDashboardView } from './types';
...
export class DashboardView extends ItemView {
    ...
    getDisplayText(): string {
        return this.dashboard.translations.t('dashboard.title');
    }
    ...
    async onOpen() {
        if (!this.isPopup) {
            ...
            this.dashboard.plugins = await this.dashboard.pluginManager.getAllPlugins();
            
            // Utiliser la fonction utilitaire
            await this.dashboard.renderContent(container);
        }
    }
    ...
    async refresh() {
        const contentEl = this.containerEl.querySelector('.pluginflowz-content');
        if (contentEl) {
            await this.dashboard.renderContent(contentEl);
        }
    }
}
...
export class Dashboard extends Component {
    ...
    private currentViewMode: TDashboardView;
    private selectedStatuses: Set<TPluginStatus>;
    ...
    constructor(
        ...
        Settings.loadSettings().then(settings => {
            this.selectedStatuses = new Set(settings.selectedStatuses || []);
            this.currentViewMode = settings.currentViewMode || 'cards';
        });
    }
    ...
    private async renderContent(container: HTMLElement) {
        ...
        this.registerDomEvent(viewButton, 'click', async () => {
            ...
            // Sauvegarder le mode de vue dans les settings
        ...
        statuses.forEach(status => {
            const tag = new Tag(statusContainer, status, async () => {
                ...
                const tagElement = statusContainer.querySelector(`.pluginflowz-tag-status.${status}`) as HTMLElement;
                if (tagElement) {
                    tagElement.toggleClass('selected', this.selectedStatuses.has(status));
                }

                // Filtrer les plugins par statut
                this.applyStatusFilters(dashboardContainer);
            }, `pluginflowz-filter-tag pluginflowz-tag-status ${status}`);

            // Réappliquer la sélection depuis l'état
            if (this.selectedStatuses.has(status)) {
                const tagElement = statusContainer.querySelector(`.pluginflowz-tag-status.${status}`) as HTMLElement;
                if (tagElement) {
        ...
        // Réappliquer les filtres après le rendu initial
    ...
    private applyStatusFilters(dashboardContainer: HTMLElement) {
        const contentContainer = dashboardContainer.querySelector('.pluginflowz-content') as HTMLElement;
        if (contentContainer) {
            contentContainer.empty();
            let filteredPlugins = this.plugins;

            // Utiliser directement selectedStatuses au lieu de chercher dans le DOM
            if (this.selectedStatuses.size > 0) {
                filteredPlugins = this.plugins.filter(p => 
                    p.status.some(s => this.selectedStatuses.has(s as TPluginStatus))
                );
            }
